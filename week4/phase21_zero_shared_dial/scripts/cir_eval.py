"""
Phase 21: identical copy of phase 12's shared CIR benchmark loading +
Recall@K evaluator + topk_for_query helper (`week5/phase17_dedicated_capacity_generalization/scripts/cir_eval.py`
extended this with `topk_for_query`; reused verbatim here), same reuse
pattern every phase since 12b has used -- BENCHMARK_JSON points directly at
phase 12's original `cir_benchmark.json`.
"""
import json
...
PHASE12_DIR = BASE_DIR.parent.parent / "week4" / "phase12_controllable_modes"
...
def topk_for_query(pools, query, item_ids, embeddings, k=10):
    idx = {a: i for i, a in enumerate(item_ids)}
