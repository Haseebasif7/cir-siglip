"""
Phase 12c, step 4 (qualitative): same 6 example queries as phase 12/12b
(identical deterministic picking logic against the same, unchanged benchmark
file, so the same 6 outfits are selected -- direct three-way visual
comparison), top-5 results under substitute mode, complement mode, and the
interpolated blend, using the ranking-distillation checkpoint.
"""
import json
import sys
...
def pick_example_queries(queries):
    """Identical logic to phase 12's 06_qualitative_examples.py -- same
    benchmark file, same iteration order, same first-match-per-category rule,
    so this deterministically picks the same 6 outfits phase 12 used."""
    picked = {}
...
def build_grid(query, rows_config, title_map, out_path):
    ...
    fig.suptitle(f"Phase 12c (ranking distillation) -- Polyvore query outfit {query['outfit_id']}, held-out "
                 f"target category: {query['category']} (true target id {query['target_item']})", fontsize=11)
    plt.tight_layout()
...
def main():
    ...
    for cat, query in picked.items():
        ...
        out_path = OUT_DIR / f"polyvore_query_{query['outfit_id']}_{cat}_ranking_distillation.png"
