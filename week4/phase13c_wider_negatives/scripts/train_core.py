"""
Phase 13c: identical to phase 13b's training loop (CSA-Net's subspace
attention on a frozen SigLIP backbone), with exactly ONE change -- see
../phase13c_wider_negatives.md step 1 -- NUM_NEGATIVES 10 -> 20, using ALL
of each item's already-mined top-20 same-category candidates instead of a
random subset of 10. Architecture, loss, margin, aggregation, optimizer,
schedule: all unchanged from phase 13b.

Reuses phase 13's training data and mined negative candidates directly
(backbone-independent -- see phase13b's architecture_notes.md), and phase 9's
...
NUM_NEGATIVES = 20  # phase 13c's one change (was 10 in phase 13b) -- see step 1
UNIFORMITY_WEIGHT = 1.0  # unchanged from phase 13b
