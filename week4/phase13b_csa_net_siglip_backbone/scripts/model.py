"""
Phase 13b: CSA-Net's category-pair-conditioned subspace attention mechanism
on a FROZEN SigLIP backbone, instead of phase 13's fine-tuned ResNet18.

Everything downstream of the base feature (masks, attention sub-network,
L2-normalization, uniformity regularizer, outfit ranking loss) is copied
UNCHANGED from phase 13's `model.py` -- see ../architecture_notes.md for the
exact diff. Only `encode_image` changes: instead of a CNN forward pass, it's
a lookup into phase 9's already-extracted SigLIP embeddings (768-d) followed
by a trainable Linear(768, 64) projection. The SigLIP embedding itself is
never updated (no backbone to fine-tune here at all -- this project doesn't
re-run SigLIP, it reuses phase 9's frozen extraction, so there's no
`pretrained`/freeze flag to manage the way phase 13 needed one).
"""
import torch
