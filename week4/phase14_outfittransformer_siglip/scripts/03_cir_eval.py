"""
Phase 14, step 5: evaluate on phase 12's exact CIR benchmark
(`cir_benchmark.json` -- same pools/queries every configuration in this
project has been scored against). Candidate embeddings are the precomputed,
context-independent `embed_item_alone` features from step 02. Query
embeddings CANNOT be precomputed the same way -- unlike a simple mean-pooled
query vector (phase 12's generic `cir_eval.py`), this mechanism's query
representation is the outfit token's output after a transformer forward
pass over the query's actual context items, so it is computed here directly
from raw SigLIP vectors via the trained model, batched by category for
efficiency (same grouping phase 12's own `evaluate_recall` uses).
"""
import json
import sys
...
def compute_query_embeddings(model, siglip_idx, siglip_emb, query_item_lists, device):
    """query_item_lists: list of lists of item ids (already filtered to
    ones present in siglip_idx, non-empty). Returns (Q, d_embed) numpy."""
    outs = []
...
def evaluate_recall(pools, queries, candidate_idx, candidate_emb, model, siglip_idx, siglip_emb, device, ks=(10, 30, 50)):
    ...
    for cat, qidxs in by_cat.items():
        ...
        pool_emb = candidate_emb[pool_idx]  # (P, D)
        ...
        query_mat = compute_query_embeddings(model, siglip_idx, siglip_emb, query_item_lists, device)  # (Q, D)
        sims = query_mat @ pool_emb.T  # (Q, P), both L2-normalized -> cosine similarity
