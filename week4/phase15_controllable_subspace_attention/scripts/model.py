"""
Phase 15: extend phase 13b's CSA-Net-on-frozen-SigLIP (`CSANetSigLIP`) so its
attention sub-network conditions on the substitute/complement control signal
`alpha` (0 = complement-leaning, 1 = substitute-leaning), not just the
category pair. Everything except `attn_net`'s input width and the alpha
plumbing through `embed_from_feature`/the candidate-embedding helpers is
copied UNCHANGED from phase 13b's model.py: masks (near-identity init),
L2-normalization, `pairwise_distance`, `outfit_ranking_loss`,
`uniformity_loss`. See ../architecture_notes.md for the full design-decision
log, including the "alpha double-duty confound" risk (alpha is both the
attn_net input AND the outer loss-term weight -- see train_core.py's
decoupled-alpha smoke-test steps and the attention-weight-shift probe, which
exist specifically to check this doesn't let attn_net ignore alpha).

Alpha representation (decision 1): raw scalar concatenated onto attn_net's
input (`2*num_categories + 1` = 23-dim), matching CSA-Net's existing
convention of feeding raw features into a tiny subnet with no embedding
step. Revisit with a small learned embedding only if the smoke-test probe
shows attn_net isn't using alpha.
"""
import torch
...
NUM_CATEGORIES = 11
NUM_SUBSPACES = 5  # k=5, paper section 4.2, unchanged from phase 13/13b
EMBED_DIM = 64  # unchanged from phase 13/13b
SIGLIP_DIM = 768  # phase 9's embeddings/siglip_base.npz
MARGIN = 0.3  # unchanged from phase 13/13b
ATTN_HIDDEN = 64  # unchanged from phase 13/13b
...
class CSANetSigLIPControllable(nn.Module):
    def __init__(self, num_categories=NUM_CATEGORIES, num_subspaces=NUM_SUBSPACES,
        ...
        # Only change vs. phase 13b: input width 2*num_categories -> 2*num_categories + 1
        # (raw alpha scalar concatenated alongside the two category one-hots).
        ...
        # Same near-identity init as phase 13/13b, same reason (dead-gradient fix).
    ...
    def attention_weight_shift(self, cat_s_onehot, cat_t_onehot):
        """Direct probe for the alpha double-duty confound (architecture_notes.md):
        for a fixed sample of (cat_s, cat_t) pairs, sweep the forward-pass alpha
        alone from 0 to 1 (no loss involved) and return the mean L1 distance
        between the alpha=0 and alpha=1 attention-weight vectors. Near-zero means
        attn_net isn't conditioning on alpha in a way that matters."""
        with torch.no_grad():
...
def uniformity_loss(f, t=2.0):
    """Unchanged from phase 13/13b -- Wang & Isola (2020) alignment/uniformity
    regularizer. Re-verified (not assumed) whether still needed here, see
    training_log.md."""
    n = f.shape[0]
...
def ranking_distillation_loss(z_anchor, z_neighbors, teacher_sims, tau=0.07):
    ...
    loss, adapted to take already-computed CSA-Net embeddings instead of
    calling `ControllableProjectionHead` internally.
