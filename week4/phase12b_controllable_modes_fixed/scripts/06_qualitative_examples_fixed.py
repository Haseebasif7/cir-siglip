"""
Phase 12b, step 5: same 6 example queries as phase 12 (identical
deterministic picking logic against the same, unchanged benchmark file, so
the same 6 outfits are selected -- direct before/after visual comparison),
top-5 results under substitute mode, complement mode, and the interpolated
blend, using the newly retrained `controllable_modes_fixed.pt` checkpoint.
"""
import json
import sys
...
def build_grid(query, rows_config, title_map, out_path):
    ...
    fig.suptitle(f"Phase 12b (fixed) -- Polyvore query outfit {query['outfit_id']}, held-out target "
...
def main():
    ...
    for cat, query in picked.items():
        ...
        out_path = OUT_DIR / f"polyvore_query_{query['outfit_id']}_{cat}_fixed.png"
