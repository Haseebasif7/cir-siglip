"""11-point alpha sweep, identical methodology to phase 16's
04_evaluate_alpha_sweep.py (same gallery -- reused directly from phase 16's
folder, no need to rebuild, it's just raw SigLIP embeddings independent of
any trained model -- same queries, same K values, same axis checks), so the
comparison to phase 16 is direct and complete.
"""
import ast
import sys
...
GALLERY_NPZ = PHASE16_DIR / "data" / "candidate_gallery.npz"  # reused directly, model-independent
...
# phase 16's own endpoint numbers, for direct side-by-side comparison in results_table.md
PHASE16_REFERENCE = {
...
def main():
    ...
    # "clearly passes" bar for the phase's own success condition: a real, non-noise gap.
    # Use phase 16's own axis-1 near-tie (~0.001) as the noise floor reference -- require at
    # least a 5x larger gap than that to call it "clear", not just "technically positive".
    axis1_gap_5 = row_rel["hit_rate@5"] - row_tail["hit_rate@5"]
    axis1_gap_10 = row_rel["hit_rate@10"] - row_tail["hit_rate@10"]
    axis1_clear = axis1_gap_5 > 0.02 and axis1_gap_10 > 0.02  # phase 16's own gap was ~0.001, noise-level

    lines = ["# Phase 16c: Results Table\n", "## Retrieval accuracy and axis checks across the alpha sweep\n"]
    ...
    lines.append("| | Phase 16 tail (a=0) | Phase 16 relevance (a=1) | Phase 16c tail (a=0) | "
                 "Phase 16c relevance (a=1) |")
    lines.append("|---|---|---|---|---|")
    for K in K_VALUES:
        lines.append(f"| Hit Rate@{K} | {PHASE16_REFERENCE[0.0][f'hit_rate@{K}']:.4f} | "
                     f"{PHASE16_REFERENCE[1.0][f'hit_rate@{K}']:.4f} | {row_tail[f'hit_rate@{K}']:.4f} | "
                     f"{row_rel[f'hit_rate@{K}']:.4f} |")
    for K in K_VALUES:
        lines.append(f"| Mean ref_count@{K} | {PHASE16_REFERENCE[0.0][f'mean_ref_count@{K}']:.2f} | "
                     f"{PHASE16_REFERENCE[1.0][f'mean_ref_count@{K}']:.2f} | {row_tail[f'mean_ref_count@{K}']:.2f} | "
                     f"{row_rel[f'mean_ref_count@{K}']:.2f} |")
    lines.append("")
    ...
    lines.append(f"- **Axis 1, CLEAR-GAP bar (this phase's own success condition -- a real, non-noise gap, not "
                 f"just movement in the right direction; phase 16's own gap was ~0.001, noise-level; requiring "
                 f"a gap > 0.02 here to count as 'clear'): {'CLEAR PASS' if axis1_clear else 'NOT CLEAR'}** -- "
                 f"gap@5={axis1_gap_5:.4f}, gap@10={axis1_gap_10:.4f}")
