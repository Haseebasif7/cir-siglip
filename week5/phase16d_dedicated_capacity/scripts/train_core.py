...
phase 16d, imported by both 01_smoke_test.py and 02_train.py -- same
train_core.py sharing convention phases 16/16c used.

Identical batch-construction structure to phase 16c: relevance and
tail-exposure modes train on two DIFFERENT edge populations (phase 7's
full 76,293 also_buy edges vs. phase 16c's 37,922 attribute-based pairs,
REUSED DIRECTLY from that phase's own data file, not rebuilt), two
independent batches per step, one epoch defined as one pass over the
larger (relevance) population, the smaller (tail) population cycled.

The one real difference from phase 16c: `grad_norm_check` now measures
gradient into `model.shared.parameters()` (the new architecture's only
shared submodule) instead of `model.net.parameters()`, since that's the
only point where the two objectives' gradients still interact under the
dedicated-capacity architecture.
"""
import random
...
from model import DedicatedCapacityHead, mean_pairwise_cosine, mnrl_loss, uniformity_loss
...
ATTRIBUTE_EDGES_JSON = PHASE16C_DIR / "data" / "attribute_pairs.json"  # reused directly, not rebuilt
...
def grad_norm_check(model, embeddings, idx, rel_edges, tail_edges, all_positive_targets, item_ids, rng, weight_tail):
    """Measures gradient into `model.shared.parameters()` -- the only
    shared submodule in the dedicated-capacity architecture, unlike phase
    16/16c's `model.net.parameters()` (the whole shared trunk there)."""
    rel_order = list(range(len(rel_edges)))
    rng.shuffle(rel_order)
    ...
    model.train()
    rel_loss, _ = loss_for_batch(model, embeddings, idx, rel_batch, rel_negs, all_positive_targets, alpha=1.0)
    rel_grad_norm = shared_grad_norm(rel_loss)

    tail_loss, _ = loss_for_batch(model, embeddings, idx, tail_batch, tail_negs, all_positive_targets, alpha=0.0)
    tail_grad_norm_unweighted = shared_grad_norm(tail_loss)

    tail_loss2, _ = loss_for_batch(model, embeddings, idx, tail_batch, tail_negs, all_positive_targets, alpha=0.0)
    tail_grad_norm_weighted = shared_grad_norm(weight_tail * tail_loss2)
