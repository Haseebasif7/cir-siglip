...
no embedding collapse (checked for BOTH heads independently, and for the
shared layer's output implicitly via the heads) and reasonable gradient
balance into the shared layer before committing to the full run."""
import random
import sys
...
def main():
    ...
    print(f"Loaded {len(item_ids)} embeddings, {len(rel_train)} relevance train edges, "
          f"{len(tail_train)} tail train edges (reused from phase 16c). Device: {DEVICE}")
    ...
    print(f"\nFinal mean pairwise cosine -- relevance head: {final_rel:.4f}, tail head: {final_tail:.4f}")
    ...
    lines = [
        ...
        f"(reused from phase 16c), {SMOKE_EPOCHS} epochs.\n",
    ...
    lines.append(f"\n**Collapse detected: {collapse_detected}** (mean pairwise cosine > 0.9 threshold, checked "
                 "independently for both dedicated heads)")
    lines.append(f"\n**Recommendation: {'enable' if collapse_detected else 'leave OFF'} the uniformity "
                 "regularizer for the full training run (02_train.py).**")
    report.write_text("\n".join(lines))
