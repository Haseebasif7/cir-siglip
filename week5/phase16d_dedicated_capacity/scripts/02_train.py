"""Full training run: fresh loss-balancing calibration for the SHARED LAYER
specifically (model.shared -- the only point where the two objectives'
gradients now interact under the dedicated-capacity architecture), then
the full train/val loop with early stopping. Same dual-independent-batch
structure as phase 16c.
"""
import json
import random
import sys
...
UNIFORMITY_ENABLED = False  # per 01_smoke_test.py's finding: no collapse sign, stays off
...
def main():
    ...
    print(f"Grad norm into SHARED LAYER (model.shared) -- relevance: {rel_gn:.6f}, "
    ...
    lines = [
        ...
        f"Measured fresh on the freshly-initialized (untrained) `DedicatedCapacityHead`, seed={SEED} -- not "
        "assumed from phases 16/16c's weights, per this project's standing rule: both the architecture and "
        "the point of interaction between the two objectives changed.\n",
        ...
        f"- Tail-exposure loss (MNRL/InfoNCE, unweighted, attribute-pair set reused from phase 16c): "
        f"{initial_tail:.4f}",
        ...
        "Gradient L2-norm into `model.shared`'s parameters (the ONLY shared submodule in this architecture "
        "-- unlike phases 16/16c, where the whole 768->256->128 trunk was shared), each loss term isolated, "
        "one fixed batch of 128 anchor edges per population:\n",
    ...
    if 0.1 <= ratio_after <= 10:
        lines.append(f"**Verification PASSED**: after weighting, the tail-exposure loss's gradient into the "
                      f"shared layer is within a reasonable order of magnitude of the relevance loss's "
        ...
        lines.append(f"**Verification DID NOT FULLY PASS**: even after weighting, the two gradients remain "
                      ...
                      "reporting this discrepancy plainly in phase16d_notes.md.")
    lines.append("")
    ...
    torch.save(model.state_dict(), MODELS_DIR / "dedicated_capacity.pt")
    print(f"Saved best checkpoint to {MODELS_DIR / 'dedicated_capacity.pt'} (best_val_rel={best_val_rel:.4f})")
