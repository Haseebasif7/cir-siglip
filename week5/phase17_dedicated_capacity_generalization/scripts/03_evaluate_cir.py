"""
Phase 17, step 5.2 (part 1): re-run the exact same CIR benchmark evaluation
as phases 12/12b/12c (identical harness, pointed directly at phase 12's
cir_benchmark.json) on this phase's dedicated-capacity checkpoint, with
phase 12/12b/12c's own numbers carried forward for a direct four-way
comparison at the two pure endpoints and the 0.5 blend.
"""
import json
import sys
...
PHASE12C_NUMBERS = {
    "Substitute mode": {10: 0.0667, 30: 0.1307, 50: 0.1734},
    "Complement mode": {10: 0.0971, 30: 0.1875, 50: 0.2471},
    "Blend (0.5)": {10: 0.0893, 30: 0.1695, 50: 0.2255},
}
...
def main():
    ...
    with open(BASE_DIR / "data" / "phase17_recall.json", "w") as f:
    ...
    lines = [
        ...
        "",
        f"Same benchmark as phases 12/12b/12c "
        f"(`week4/phase12_controllable_modes/data/cir_benchmark.json`, unchanged): "
    ...
    lines.append("| | | | | |")
    for label, alpha in configs:
        lines.append(fmt_row(f"Phase 12 (shared trunk, batch-local pairwise): {label}", PHASE12_NUMBERS[label], 29681))
        lines.append(fmt_row(f"Phase 12b (shared trunk, PCA-128 target): {label}", PHASE12B_NUMBERS[label], 29681))
        lines.append(fmt_row(f"Phase 12c (shared trunk, ranking distillation): {label}", PHASE12C_NUMBERS[label], 29681))
        new_recall, n, n_skip = new_rows[label]
        lines.append(fmt_row(f"**Phase 17 (dedicated capacity, ranking distillation): {label}**", new_recall, n))
        lines.append("| | | | | |")
    lines.append(fmt_row("OutfitTransformer (literature anchor, not independently reproduced)", LITERATURE_ANCHOR))
    lines.append("")
    lines.append(
        "**Reading this table**: as established since phase 12, Recall@K alone is not "
        "sensitive enough to tell whether the two modes are behaviorally distinct on its own "
        "-- see the alpha-sweep table below (axis checks, mode overlap, overlap with raw "
        "SigLIP) for the decisive comparison, and `smoothness_check.md` for whether this is a "
        "genuine, continuously usable dial."
    )
    lines.append("")
