"""11-point alpha sweep (phase 12d's convention: 0.0-1.0, step 0.1). For each
alpha, project the trained model's embeddings for the 26,591-item candidate
gallery and the 1,872-item phase 1b query set, retrieve top-50 gallery items
per query (self-excluded when a query asin also appears in the gallery,
since the gallery was deliberately built to include phase 1b's items), and
cache the retrievals for 05/06 to reuse. Computes Hit-Rate@5/@10 against
phase 1b's also_buy/also_viewed ground truth (this project's standing Amazon
convention, `load_relatedness()` reused verbatim from phase 1b's own
03_retrieval_eval.py) and the two axis checks the brief requires.
"""
import ast
import sys
...
K_CACHE = 50
K_VALUES = [5, 10]  # this project's standing Amazon Hit-Rate convention


def load_relatedness():
    """Reused verbatim from week2/phase1b_category_balanced/scripts/03_retrieval_eval.py."""
...
def main():
    ...
    print(f"Gallery: {len(gallery_asins)} items. Queries: {len(query_asins)}. "
    ...
    for ai, alpha in enumerate(ALPHAS):
        with torch.no_grad():
            z_gallery = model(gallery_t, alpha=alpha)  # (G, 128)
            z_query = model(query_t, alpha=alpha)      # (Q, 128)
            sims = z_query @ z_gallery.T                # (Q, G)
            if self_match_rows:
        ...
        retrieved_asins = gallery_asins[topk_idx_np]  # (Q, K_CACHE)
    ...
    np.savez(
        ...
        query_asins=query_asins,
        retrieved_asins=all_retrieved_asins,
        retrieved_sims=all_retrieved_sims,
    )
    print(f"Cached retrievals: {OUT_CACHE}")

    # axis checks at the endpoints
    ...
    print(f"Axis 1: {'PASS' if axis1_pass else 'FAIL'}, Axis 2: {'PASS' if axis2_pass else 'FAIL'}")
