"""
Phase 7, step 5: train the compatibility projection head on frozen SigLIP
embeddings from the new training pool.

Trains TWO model variants for the ablation (step 6 will compare them):
  Model A: random negatives only (R=8, H=0)
  Model B: random + explicit hard negatives (R=4, H=4)
Total explicit-negative count is fixed at 8 for both, so the ablation
isolates negative TYPE, not negative COUNT. Both models use the identical
seed/batch order -- only the hard-negative component differs.

Training is pure integer-indexing over a preloaded embedding matrix (no
image I/O, no encoder forward pass, since SigLIP stays frozen) -- a plain
hand-rolled loop over shuffled edge indices, consistent with this project's
established preference for dependency-light code (no DataLoader, no sklearn).
"""
import json
import random
...
def sample_random_negatives(rng, anchor, positive_set, n_items, idx_to_asin, all_asins_set, k):
    """Rejection-sample k random negatives for `anchor`, excluding itself and its positives."""
    negs = []
    ...
    while len(negs) < k and tries < max_tries:
        ...
        if cand not in exclude and cand not in negs:
            negs.append(cand)
        tries += 1
    while len(negs) < k:  # extremely rare fallback if rejection sampling stalls
        cand = idx_to_asin[rng.randrange(n_items)]
        negs.append(cand)
    return negs
...
def build_negatives_for_edges(edges, hard_neg, all_positive_targets, asins_arr, rng, R, H):
    """Returns a list (parallel to edges) of negative-asin lists, length R+H each."""
    idx_to_asin = asins_arr
    n_items = len(asins_arr)
    out = []
    for anchor, _ in edges:
        positive_set = all_positive_targets.get(anchor, set())
        rand_negs = sample_random_negatives(rng, anchor, positive_set, n_items, idx_to_asin, None, R)
        if H > 0:
            candidates = hard_neg.get(anchor, [])
            if len(candidates) >= H:
                hard_negs = rng.sample(candidates, H)
            else:
                # fallback: anchor has too few hard-negative candidates (e.g. all
                # neighbors excluded as positives or too-similar) -- pad with
                # extra random negatives instead of crashing or reusing duplicates
                hard_negs = list(candidates)
                extra_needed = H - len(hard_negs)
                hard_negs += sample_random_negatives(
                    rng, anchor, positive_set | set(hard_negs), n_items, idx_to_asin, None, extra_needed
                )
        else:
            hard_negs = []
...
def run_epoch_train(model, optimizer, embeddings, idx, edges, hard_neg, all_positive_targets,
    ...
    for start in range(0, len(order), BATCH_SIZE):
        ...
        extra_emb = torch.tensor(embeddings[np.array(extra_idx)], device=DEVICE)  # (B, R+H, 768)
...
def train_model(name, R, H, embeddings, idx, asins_arr, train_edges, val_edges, hard_neg, all_positive_targets):
    ...
    val_rng = random.Random(SEED + 1000)  # separate, frozen once for val negatives

    model = ProjectionHead().to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)

    # freeze val negatives once so the val-loss curve is stable enough to drive early stopping
...
def main():
    ...
    print(f"Loaded {len(asins_arr)} embeddings, {len(train_edges)} train edges, {len(val_edges)} val edges.")
