"""
Phase 32: Modal GPU training app, copied unchanged from phase 31
(week7/phase31_fair_baseline_outfittransformer/scripts/modal_app.py) -- same
architecture, same winning hyperparameters, same volume, same credit-safety
pattern. The only difference is the app name ("phase32-partial-ensemble-ot",
own deployment, cleanly separated from phase 31's) and the config-name
prefix (`ot32_` instead of `ot31_`, so checkpoints/status files never
collide with phase 31's own on the shared volume). Per this phase's own
brief: no architecture changes, no scale testing -- this phase trains
additional SEEDS of phase 31's exact winning configuration only, reusing
"phase27-text-category-data" (already holds every data file needed:
siglip_base.npz, text_embeddings.npz, training_data.json,
cir_val_benchmark.json, cir_train_benchmark.json, cir_test_benchmark.json --
...
app = modal.App("phase32-partial-ensemble-ot")
...
def train_one(config: dict) -> dict:
    ...
    assert cfg["name"].startswith("ot32_"), "every phase32 config name must be prefixed ot32_"
...
def smoke_test():
    ...
    for input_mode in ("image", "image_text"):
        cfg = {"name": f"ot32_smoke_{input_mode}", "input_mode": input_mode,
